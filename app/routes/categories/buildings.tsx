...
import type { Building, User } from "../../types/management";

export default function BuildingsCategoryPage() {
  ...
  const [editing, setEditing] = useState<Building | null | undefined>(undefined);
  ...
  async function loadBuildings() {
    ...
    setError("");
    try {
      setBuildings(await apiListRequest<Building>("/buildings?size=1000").catch(async () => apiListRequest<Building>("/buildings")));
    } catch (reason) {
      const err = reason as ApiError;
      if (err.status === 401) navigate("/login");
      else setError(err.message || "Không thể tải danh sách tòa nhà.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const visibleBuildings = useMemo(() => {
    ...
    return term
      ? buildings.filter((b) =>
          `${b.buildingCode} ${b.name} ${b.description || ""}`.toLowerCase().includes(term)
        )
      : buildings;
  }, [buildings, search]);
  ...
  const totalItems = visibleBuildings.length;
  const totalPages = Math.ceil(totalItems / pageSize) || 1;
  const paginatedBuildings = useMemo(() => {
    const start = (currentPage - 1) * pageSize;
    return visibleBuildings.slice(start, start + pageSize);
  }, [visibleBuildings, currentPage, pageSize]);

  async function confirmDeleteBuilding() {
    if (!deletingBuilding) return;
    setDeleting(true);
    try {
      await apiRequest<string>(`/buildings/${deletingBuilding.id}`, { method: "DELETE" });
      setDeletingBuilding(null);
      await loadBuildings();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Không thể xóa tòa nhà.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  return (
    <AppShell title="Danh mục Tòa nhà" description="Quản lý danh sách tòa nhà giảng đường, khu hiệu bộ và cơ sở vật chất.">
        ...
        <div className="flex flex-col gap-4 border-b border-slate-200 dark:border-white/10 p-6 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-lg text-slate-900 dark:text-white">Danh sách Tòa nhà</h2>
            </div>
          </div>
          ...
          {isAdmin && (
            <button
              ...
              className="flex items-center gap-2 rounded-2xl bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-600 px-5 py-3 text-xs font-bold text-white shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:brightness-110 active:scale-[0.98] transition-all cursor-pointer"
            >
              <PlusIcon size={16} />
        ...
        <div className="border-b border-white/5 p-4 bg-slate-50 dark:bg-slate-950/40">
          <div className="relative flex items-center sm:max-w-md w-full">
            ...
            <input
              ...
              placeholder="Tìm theo mã tòa nhà, tên tòa nhà, ghi chú..."
            />
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left text-xs">
            <thead className="bg-slate-100 dark:bg-slate-950/80 text-[11px] font-bold uppercase tracking-wider text-slate-700 dark:text-cyan-300 border-b border-slate-200 dark:border-white/10">
              <tr>
                <th className="px-6 py-4">Mã tòa</th>
                ...
                paginatedBuildings.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-6 py-4">
                      <span className="rounded-full border border-cyan-300 dark:border-cyan-500/30 bg-cyan-50 dark:bg-cyan-500/10 px-3 py-1 text-[11px] font-mono font-bold text-cyan-800 dark:text-cyan-300">
                        {b.buildingCode}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-bold text-slate-900 dark:text-slate-100">{b.name}</td>
                    <td className="px-6 py-4 font-bold text-slate-900 dark:text-white">{b.totalFloors || 5} Tầng</td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-300 dark:border-emerald-500/30 bg-emerald-50 dark:bg-emerald-500/10 px-3 py-1 text-[11px] font-bold text-emerald-800 dark:text-emerald-300">
                        <span className="size-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse" />
                        {b.status || "Hoạt động"}
                      </span>
                    </td>
                    <td className="px-6 py-4 font-medium text-slate-600 dark:text-slate-400">{b.description || "—"}</td>
                    {isAdmin && (
                      <td className="px-6 py-4 text-right">
                        <ActionIcon label="Sửa tòa nhà" color="blue" onClick={() => setEditing(b)}>
                          <path d="M4 16.5V20h3.5L18 9.5 14.5 6 4 16.5Z" />
                          <path d="m13.5 7 3.5 3.5" />
                        </ActionIcon>
                        <ActionIcon label="Xóa tòa nhà" color="red" onClick={() => setDeletingBuilding(b)}>
                          <path d="M4 7h16" />
      ...
      {deletingBuilding && (
        <ConfirmModal
          ...
          message={`Bạn có chắc chắn muốn xóa tòa nhà ${deletingBuilding.name} (${deletingBuilding.buildingCode})? Dữ liệu sẽ không thể hoàn tác.`}
...
function ActionIcon({
  ...
  const tones =
    color === "blue"
      ? "text-cyan-700 dark:text-cyan-400 hover:bg-cyan-50 dark:hover:bg-cyan-500/10 hover:text-cyan-800 dark:hover:text-cyan-300"
