...
export default function ExamSchedulePage() {
  ...
  async function loadExamSchedules() {
    ...
    try {
      if (isAdmin) {
        setExams(await apiListRequest<ExamSchedule>("/exam-schedules?size=200"));
      } else {
        setExams(
          await apiListRequest<ExamSchedule>(
            `/exam-schedules/my?semester=${semester}&academicYear=${academicYear}`
          ).catch(async () => await apiListRequest<ExamSchedule>("/exam-schedules?size=200"))
        );
      }
    } catch (reason) {
      const apiError = reason as ApiError;
      if (apiError.status === 401) navigate("/login");
      else setError(apiError.message || "Không tải được lịch thi.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleDeleteExamConfirm() {
    ...
    try {
      ...
      alert((reason as ApiError).message || "Lỗi khi xóa ca thi.");
    } finally {
      setDeleting(false);
    }
  }
  ...
  return (
    <AppShell title="Màn Lịch thi" description="Lịch thi kết thúc học phần dành cho sinh viên và cán bộ coi thi.">
              ...
              {loading ? (
                <tr>
                  <td colSpan={isAdmin ? 6 : 5} className="py-12 text-center text-xs text-slate-400">
                    Đang tải dữ liệu lịch thi cá nhân...
                  </td>
                </tr>
              ) : exams.length === 0 ? (
                <tr>
                ...
                exams.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="px-6 py-4 font-semibold text-slate-900 dark:text-white">
                      <div>{item.name}</div>
                      <div className="text-xs text-cyan-700 dark:text-cyan-700 dark:text-cyan-300 font-bold font-mono font-normal">Mã: {item.examCode}</div>
                    </td>
                    ...
                    {/* Chỉ Admin mới có nút Sửa và Xóa */}
                    {isAdmin && (
